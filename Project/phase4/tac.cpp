...
tac_opd *tac_opd_from_str(char *tk) {
    int i;
...
tac *tac_from_line(char *ln) {
    ...
    if (count == 2) {
        ...
        if (strcmp(tokens[0], "LABEL") == 0) {
            ...
            if (tokens[0][0] == '*') {
                t1 = tac_opd_from_str(tokens[0] + 1);
                t2 = tac_opd_from_str(tokens[2]);
                code = tac_init_deref(t1, t2);
            } else if (tokens[2][0] == '*') {
                t1 = tac_opd_from_str(tokens[0]);
                t2 = tac_opd_from_str(tokens[2] + 1);
        ...
        t1 = tac_opd_from_str(tokens[1]);
        t2 = tac_opd_from_str(tokens[3]);
        t3 = tac_opd_from_str(tokens[5]);
